...
def solve_intercept(fx: float, fy: float, tx: float, ty: float, orbiting: bool, omega: float, ships: int, source_rad: float, target_rad: float, iterations: int = 25):
    if not orbiting:
        t = travel_time(fx, fy, tx, ty, ships, source_rad, target_rad)
        return tx, ty, t, True
    ...
    for _ in range(iterations):
        ...
        t2 = travel_time(fx, fy, ix, iy, ships, source_rad, target_rad)
        if abs(t2 - t) < 0.05:
            break
        t = t2
    valid = False
...
def solve_intercept_at_all_costs(fx: float, fy: float, tx: float, ty: float, orbiting: bool, omega: float, ships: int, source_rad: float, target_rad: float, time_limit: int = 50):
    if not orbiting:
        t = travel_time(fx, fy, tx, ty, ships, source_rad, target_rad)
        return tx, ty, t
    # theta = math.atan2(ty - SUN_Y, tx - SUN_X)
    ...
    for _ in range(time_limit):
        ix, iy = predict_orbit(tx, ty, omega, t)
        time_taken = travel_time(fx, fy, ix, iy, ships, source_rad, target_rad)
