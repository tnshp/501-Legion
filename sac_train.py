...
class SACTrainer:
    ...
    def __init__(
        ...
        # ── replay buffer ─────────────────────────────────────────────────────
        # Under TD(λ), round capacity down to a whole number of env-interleaved
        # rows so the per-env stride stays aligned across the circular wrap (the
        # λ-return cache promotes per-env strided blocks — see _build_lambda_cache).
        ...
        print(f"Using ReplayBuffer ({replay_buffer_size} capacity"
              + (f", env_stride={self._lambda_env_stride}" if use_lambda_returns else "")
              + ")")
